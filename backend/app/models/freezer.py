from sqlalchemy import (
    ...
    ForeignKey, DateTime, Boolean, CheckConstraint, UniqueConstraint
)
...
class Freezer(Base):
    ...
    # Per-drawer capacity — fixed by the physical drawer dimensions.
    # A drawer holds EITHER type, never both. First container locks the type;
    # emptying the drawer makes it flexible again.
    study_sample_capacity  = Column(Integer, nullable=False)
    stdqc_capacity         = Column(Integer, nullable=False)
    ...
    __table_args__ = (
        ...
        CheckConstraint("study_sample_capacity > 0", name="check_positive_study_capacity"),
        CheckConstraint("stdqc_capacity > 0",        name="check_positive_stdqc_capacity"),
    )
